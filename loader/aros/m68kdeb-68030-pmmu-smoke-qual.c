/* M1.3b.4b.6b-PMMU isolated FS-UAE/AROS same-page supervisor-data read qualification. */
#include <dos/dos.h>
...
int main(void)
{
    ...
    *scratch = 0xffffU;
    ...
    marker("SYS:m1-3b4b6b-pmmu-armed.marker",
           "68030 same-page supervisor-data read control armed\n");
    ...
    if (rc == 0 && *scratch == 0x1111U) {
        marker("SYS:m1-3b4b6b-pmmu-returned.marker",
               "68030 same-page supervisor-data read control returned\n");
        marker("SYS:m1-3b4b6b-pmmu-pass.marker",
               "68030 same-page supervisor-data read control passed\n");
        Printf("M68KDEB_PMMU_SMOKE_PASS\n");
    } else {
        marker("SYS:m1-3b4b6b-pmmu-fail.marker",
               "68030 same-page supervisor-data read control failed\n");
        rc = 20;
    }
