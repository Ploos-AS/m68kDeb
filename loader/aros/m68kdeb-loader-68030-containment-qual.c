/*
 * M1.3b.4b.5z transition-containment qualification harness.
 *
 * CI/emulator-only. The production loader is included unchanged and only the
 * completed takeover-request builder is interposed. After the same actual
 * payload, identity, trampoline-copy, layout and ABI checks as 5y, this harness
 * arms a durable marker and transfers into the exact copied transition entry.
 * The real transition intentionally disables cache/MMU state, executes PFLUSHA
 * and loops locally. It contains no Linux jump. A return is always a failure.
 */
...
static struct ExecBase *m68kdeb_5z_sysbase;

static int m68kdeb_5z_build_takeover_request(
...
#define m68kdeb_build_takeover_request m68kdeb_5z_build_takeover_request
#define main m68kdeb_5z_embedded_loader_main
#include "m68kdeb-loader.c"
#undef main
#undef m68kdeb_build_takeover_request

static struct MemHeader *m68kdeb_5z_owner(ULONG base, ULONG bytes)
{
    struct MemHeader *mh;
    ULONG end;

    if (!m68kdeb_5z_sysbase || !base || !bytes || base > 0xffffffffUL - bytes)
        return NULL;
    end = base + bytes;
    for (mh = (struct MemHeader *)m68kdeb_5z_sysbase->MemList.lh_Head;
...
static int m68kdeb_5z_bytes_equal(const UBYTE *a, const UBYTE *b, ULONG bytes)
{
    ULONG i;
    for (i = 0; i < bytes; i++)
        if (a[i] != b[i]) return 0;
...
static int m68kdeb_5z_offsets_valid(ULONG blob_len)
{
...
static int m68kdeb_5z_arm_marker(void)
{
    static const char text[] = "M1.3b.4b.5z transition armed\n";
    BPTR fh = Open("SYS:m1-3b4b5z-transition-armed.marker", MODE_NEWFILE);
    LONG wrote;
    ...
    struct m68kdeb_takeover_request *req)
{
    ...
    rc = m68kdeb_build_takeover_request(layout, bootinfo_bytes,
                                        ...
                                        cpu, mmu, req);
    Printf("M68KDEB_5Z_REQUEST_BUILD_RC=%ld\n", (LONG)rc);
    ...
    layout_owner = m68kdeb_5z_owner(req->layout_base, combined);
    initramfs_owner = m68kdeb_5z_owner(req->initramfs_addr,
                                       req->initramfs_bytes);
    ...
    Printf("containment_tc_rc=%ld\n", (LONG)tc_rc);
    Printf("containment_tc=0x%08lx\n", tc);
    if (tc_rc != 0)
    ...
    entry_off = (ULONG)m68kdeb_trampoline_entry_offset;
    abi_off = (ULONG)m68kdeb_trampoline_abi_offset;
    tc_zero_off = (ULONG)m68kdeb_trampoline_tc_zero_offset;
    if (!blob_len || blob_len > PAGE_SIZE || !m68kdeb_5z_offsets_valid(blob_len))
    ...
    trampoline_page = (UBYTE *)(((ULONG)trampoline_raw + PAGE_SIZE - 1UL) &
                                ~(PAGE_SIZE - 1UL));
    trampoline_owner = m68kdeb_5z_owner((ULONG)trampoline_page, PAGE_SIZE);
    if (!trampoline_owner) {
    ...
    if (!m68kdeb_5z_bytes_equal(trampoline_page,
    ...
    Printf("M68KDEB_5Z_TRAMPOLINE_COPY_VERIFIED bytes=%lu\n", blob_len);
    ...
    Printf("transition_entry_transfer=ARMED\n");
    Printf("M68KDEB_5Z_TRANSITION_ARMED\n");
    Flush(Output());

    if (m68kdeb_5z_arm_marker() != 0) {
        Printf("M68KDEB_5Z_ERROR armed_marker\n");
        Flush(Output());
        FreeMem(trampoline_raw, TRAMPOLINE_RAW_BYTES);
        return M68KDEB_TAKEOVER_BAD_ARGUMENT;
    }

    /* Non-returning by contract. The exact copied entry starts at offset 0. */
    rc = m68kdeb_68030_jump_transition_entry(
        (APTR)(trampoline_page + entry_off), req);

    Printf("M68KDEB_5Z_UNEXPECTED_RETURN rc=%ld\n", (LONG)rc);
    Flush(Output());
    FreeMem(trampoline_raw, TRAMPOLINE_RAW_BYTES);
    return M68KDEB_TAKEOVER_BAD_ARGUMENT;
}
...
int main(int argc, char **argv)
{
    ...
    m68kdeb_5z_sysbase = *((struct ExecBase **)4UL);
    Printf("M68KDEB_5Z_START\n");
    rc = m68kdeb_5z_embedded_loader_main(argc, argv);
    Printf("M68KDEB_5Z_LOADER_RETURNED_UNEXPECTEDLY rc=%ld\n", (LONG)rc);
    return rc ? rc : 20;
}
