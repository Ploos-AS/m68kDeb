/*
 * M1.3b.4b.5aa Linux-entry transfer qualification harness.
 *
 * CI/emulator-only. Production loader behavior is included unchanged; only
 * the completed takeover-request builder is interposed.  The harness repeats
 * the actual-payload identity and reversible ABI proofs, copies the separate
 * 5aa transition blob into an identity-qualified page, persists evidence, and
 * transfers to that blob.  The blob performs the already-qualified 5z
 * cache/MMU transition and then JMPs to the actual Linux entry in a1.
 */
...
extern unsigned char m68kdeb_5aa_blob[];
extern unsigned int m68kdeb_5aa_blob_len;
extern unsigned int m68kdeb_5aa_entry_offset;
extern unsigned int m68kdeb_5aa_mmu_off_offset;
extern unsigned int m68kdeb_5aa_post_mmu_offset;
extern unsigned int m68kdeb_5aa_linux_jump_offset;
extern unsigned int m68kdeb_5aa_tc_zero_offset;
extern unsigned int m68kdeb_5aa_abi_offset;
extern unsigned int m68kdeb_5aa_rev_offset;
extern unsigned int m68kdeb_5aa_end_offset;
...
static struct ExecBase *m68kdeb_5aa_sysbase;

static int m68kdeb_5aa_build_takeover_request(
...
#define m68kdeb_build_takeover_request m68kdeb_5aa_build_takeover_request
#define main m68kdeb_5aa_embedded_loader_main
#include "m68kdeb-loader.c"
#undef main
#undef m68kdeb_build_takeover_request

static struct MemHeader *m68kdeb_5aa_owner(ULONG base, ULONG bytes)
{
    struct MemHeader *mh;
    ULONG end;

    if (!m68kdeb_5aa_sysbase || !base || !bytes || base > 0xffffffffUL - bytes)
        return NULL;
    end = base + bytes;
    for (mh = (struct MemHeader *)m68kdeb_5aa_sysbase->MemList.lh_Head;
...
static int m68kdeb_5aa_bytes_equal(const UBYTE *a, const UBYTE *b, ULONG bytes)
{
    ULONG i;
    for (i = 0; i < bytes; i++)
        if (a[i] != b[i]) return 0;
...
static int m68kdeb_5aa_offsets_valid(ULONG blob_len)
{
    ULONG entry = (ULONG)m68kdeb_5aa_entry_offset;
    ULONG mmu_off = (ULONG)m68kdeb_5aa_mmu_off_offset;
    ULONG post_mmu = (ULONG)m68kdeb_5aa_post_mmu_offset;
    ULONG linux_jump = (ULONG)m68kdeb_5aa_linux_jump_offset;
    ULONG tc_zero = (ULONG)m68kdeb_5aa_tc_zero_offset;
    ULONG abi = (ULONG)m68kdeb_5aa_abi_offset;
    ULONG rev = (ULONG)m68kdeb_5aa_rev_offset;
    ULONG end = (ULONG)m68kdeb_5aa_end_offset;

    return entry == 0 &&
           entry < mmu_off && mmu_off < post_mmu && post_mmu < linux_jump &&
           linux_jump < tc_zero && tc_zero + 4UL <= abi &&
           abi < rev && rev < end && end <= blob_len;
}

static int m68kdeb_5aa_arm_marker(void)
{
    static const char text[] = "M1.3b.4b.5aa Linux entry transfer armed\n";
    BPTR fh = Open("SYS:m1-3b4b5aa-linux-entry-armed.marker", MODE_NEWFILE);
    LONG wrote;
    ...
    struct m68kdeb_takeover_request *req)
{
    ...
    ULONG combined, blob_len, abi_off, entry_off, tc_zero_off;
    ULONG tc = 0;
    APTR old_super;
    ...
    rc = m68kdeb_build_takeover_request(layout, bootinfo_bytes,
                                        ...
                                        cpu, mmu, req);
    Printf("M68KDEB_5AA_REQUEST_BUILD_RC=%ld\n", (LONG)rc);
    ...
    layout_owner = m68kdeb_5aa_owner(req->layout_base, combined);
    initramfs_owner = m68kdeb_5aa_owner(req->initramfs_addr,
                                        req->initramfs_bytes);
    ...
    Printf("linux_entry_tc_rc=%ld\n", (LONG)tc_rc);
    Printf("linux_entry_tc=0x%08lx\n", tc);
    if (tc_rc != 0)
    ...
    blob_len = (ULONG)m68kdeb_5aa_blob_len;
    entry_off = (ULONG)m68kdeb_5aa_entry_offset;
    abi_off = (ULONG)m68kdeb_5aa_abi_offset;
    tc_zero_off = (ULONG)m68kdeb_5aa_tc_zero_offset;
    if (!blob_len || blob_len > PAGE_SIZE ||
        !m68kdeb_5aa_offsets_valid(blob_len))
    ...
    trampoline_page = (UBYTE *)(((ULONG)trampoline_raw + PAGE_SIZE - 1UL) &
                                ~(PAGE_SIZE - 1UL));
    trampoline_owner = m68kdeb_5aa_owner((ULONG)trampoline_page, PAGE_SIZE);
    if (!trampoline_owner) {
    ...
    CopyMem((APTR)m68kdeb_5aa_blob, (APTR)trampoline_page, blob_len);
    if (!m68kdeb_5aa_bytes_equal(trampoline_page, m68kdeb_5aa_blob, blob_len)) {
        FreeMem(trampoline_raw, TRAMPOLINE_RAW_BYTES);
        return M68KDEB_TAKEOVER_BAD_ARGUMENT;
    }
    ...
    Printf("M68KDEB_5AA_TRAMPOLINE_COPY_VERIFIED bytes=%lu\n", blob_len);
    ...
    Printf("linux_jump_offset=%lu\n", (ULONG)m68kdeb_5aa_linux_jump_offset);
    ...
    Printf("linux_entry_transfer=ARMED\n");
    Printf("M68KDEB_5AA_LINUX_ENTRY_ARMED\n");
    Flush(Output());

    if (m68kdeb_5aa_arm_marker() != 0) {
        Printf("M68KDEB_5AA_ERROR armed_marker\n");
        Flush(Output());
        FreeMem(trampoline_raw, TRAMPOLINE_RAW_BYTES);
        return M68KDEB_TAKEOVER_BAD_ARGUMENT;
    }

    /* Non-returning if transition and Linux entry transfer are reached. */
    rc = m68kdeb_68030_jump_transition_entry(
        (APTR)(trampoline_page + entry_off), req);

    Printf("M68KDEB_5AA_UNEXPECTED_RETURN rc=%ld\n", (LONG)rc);
    Flush(Output());
    FreeMem(trampoline_raw, TRAMPOLINE_RAW_BYTES);
    return M68KDEB_TAKEOVER_BAD_ARGUMENT;
}
...
int main(int argc, char **argv)
{
    ...
    m68kdeb_5aa_sysbase = *((struct ExecBase **)4UL);
    Printf("M68KDEB_5AA_START\n");
    rc = m68kdeb_5aa_embedded_loader_main(argc, argv);
    Printf("M68KDEB_5AA_LOADER_RETURNED_UNEXPECTEDLY rc=%ld\n", (LONG)rc);
    return rc ? rc : 20;
}
