...
cp "$LOADER" "$OUT/aros-root/m68kdeb-loader-68030-containment-qual"
cp "$KERNEL" "$OUT/aros-root/vmlinux-m68k-amiga"
cp "$INITRAMFS" "$OUT/aros-root/initramfs-m68kdeb.gz"
rm -f "$OUT/aros-root/m1-3b4b5z-"*.marker "$OUT/aros-root/m68kdeb-5z-report.txt"

cat > "$OUT/aros-root/S/Startup-Sequence" <<'EOF'
C:Echo "M1.3b.4b.5z Startup-Sequence reached" >SYS:m1-3b4b5z-startup.marker
SYS:m68kdeb-loader-68030-containment-qual SYS:vmlinux-m68k-amiga SYS:initramfs-m68kdeb.gz 5 >SYS:m68kdeb-5z-report.txt
C:Echo "M1.3b.4b.5z loader returned unexpectedly" >SYS:m1-3b4b5z-returned.marker
C:Wait 300
EOF

{
  ...
  echo "role=M1.3b.4b.5z CI-only transition containment"
  ...
  sha256sum "$OUT/rom/aros-rom.bin" "$OUT/rom/aros-ext.bin" \
    "$OUT/aros-root/m68kdeb-loader-68030-containment-qual" \
...
echo 'PASS: prepared M1.3b.4b.5z transition-containment runtime'
