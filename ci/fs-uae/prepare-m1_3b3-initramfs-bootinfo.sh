...
rm -f "$OUT/aros-root/m1-3b3-"*.marker "$OUT/aros-root/m68kdeb-bootinfo-report.txt"

cat > "$OUT/aros-root/S/Startup-Sequence" <<'EOF'
C:Echo "M1.3b.3 Startup-Sequence reached" >SYS:m1-3b3-startup.marker
SYS:m68kdeb-loader SYS:vmlinux-m68k-amiga SYS:initramfs-m68kdeb.gz 5 >SYS:m68kdeb-bootinfo-report.txt
C:Echo "M1.3b.3 loader returned" >SYS:m1-3b3-loader-returned.marker
C:Wait 300
EOF

{
  ...
  echo "loader_role=kernel + initramfs load and bootinfo construction/self-check; no handoff"
...
echo 'PASS: prepared M1.3b.3 initramfs/bootinfo runtime'
