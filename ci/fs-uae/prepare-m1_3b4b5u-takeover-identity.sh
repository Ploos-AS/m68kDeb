...
PROBE=${PROBE:-out/m1-3b4b5u/m68kdeb-68030-takeover-identity-runtime}
OUT=${OUT:-out/m1-3b4b5u}
...
cp "$PROBE" "$OUT/aros-root/m68kdeb-68030-takeover-identity-runtime"
rm -f "$OUT/aros-root/m1-3b4b5u-"*.marker "$OUT/aros-root/m68kdeb-5u-report.txt"

cat > "$OUT/aros-root/S/Startup-Sequence" <<'EOF'
C:Echo "M1.3b.4b.5u Startup-Sequence reached" >SYS:m1-3b4b5u-startup.marker
SYS:m68kdeb-68030-takeover-identity-runtime >SYS:m68kdeb-5u-report.txt
C:Echo "M1.3b.4b.5u probe returned" >SYS:m1-3b4b5u-returned.marker
C:Wait 300
EOF

{
  ...
  echo "cpu=68030"
  echo "mmu=68030"
  echo "role=M1.3b.4b.5u reversible takeover identity contract"
  ...
  sha256sum "$OUT/rom/aros-rom.bin" "$OUT/rom/aros-ext.bin" "$OUT/aros-root/m68kdeb-68030-takeover-identity-runtime"
} > "$OUT/RUNTIME_PROVENANCE.txt"

echo 'PASS: prepared M1.3b.4b.5u runtime'
