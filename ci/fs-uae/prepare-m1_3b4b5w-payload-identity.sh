...
rm -rf "$OUT/aros-root" "$OUT/rom" "$OUT/download" "$OUT/aros-fetch" "$OUT/logs" "$OUT/screenshots"
...
rm -f "$OUT/aros-root/m1-3b4b5w-"*.marker "$OUT/aros-root/m68kdeb-5w-report.txt"

cat > "$OUT/aros-root/S/Startup-Sequence" <<'EOF'
C:Echo "M1.3b.4b.5w Startup-Sequence reached" >SYS:m1-3b4b5w-startup.marker
SYS:m68kdeb-loader SYS:vmlinux-m68k-amiga SYS:initramfs-m68kdeb.gz 5 >SYS:m68kdeb-5w-report.txt
C:Echo "M1.3b.4b.5w loader returned" >SYS:m1-3b4b5w-returned.marker
C:Wait 300
EOF

{
  ...
  echo "payload=actual qualified Linux kernel plus m68kDeb initramfs"
  ...
  sha256sum "$OUT/rom/aros-rom.bin" "$OUT/rom/aros-ext.bin" \
    ...
    "$OUT/aros-root/initramfs-m68kdeb.gz" "$OUT/aros-root/S/Startup-Sequence"
} > "$OUT/RUNTIME_PROVENANCE.txt"

echo 'PASS: prepared M1.3b.4b.5w actual payload identity runtime'
