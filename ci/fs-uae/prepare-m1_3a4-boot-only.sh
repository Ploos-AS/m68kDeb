...
rm -f "$OUT/aros-root/m1-3a4-startup.marker"

cat > "$OUT/aros-root/S/Startup-Sequence" <<'EOF'
C:Echo "M1.3a.4 AROS boot-only Startup-Sequence reached" >SYS:m1-3a4-startup.marker
C:Echo "m68kDeb M1.3a.4 AROS boot-only probe"
C:Wait 300
EOF
...
echo "PASS: prepared pinned AROS boot-only root"
