...
PROBE=${PROBE:-out/m1-3b4b5r/m68kdeb-68030-trampoline-copy-runtime}
OUT=${OUT:-out/m1-3b4b5r}
...
cp "$PROBE" "$OUT/aros-root/m68kdeb-68030-trampoline-copy-runtime"
rm -f "$OUT/aros-root/m1-3b4b5r-"*.marker "$OUT/aros-root/m68kdeb-5r-report.txt"

cat > "$OUT/aros-root/S/Startup-Sequence" <<'EOF'
C:Echo "M1.3b.4b.5r Startup-Sequence reached" >SYS:m1-3b4b5r-startup.marker
SYS:m68kdeb-68030-trampoline-copy-runtime >SYS:m68kdeb-5r-report.txt
C:Echo "M1.3b.4b.5r probe returned" >SYS:m1-3b4b5r-returned.marker
C:Wait 300
EOF

{
  ...
  echo "cpu=68030"
  echo "mmu=68030"
  echo "role=M1.3b.4b.5r reversible trampoline blob copy and verify"
  echo "trampoline_execute=not_attempted"
  echo "linux_handoff=not_attempted"
  sha256sum "$OUT/rom/aros-rom.bin" "$OUT/rom/aros-ext.bin" "$OUT/aros-root/m68kdeb-68030-trampoline-copy-runtime"
} > "$OUT/RUNTIME_PROVENANCE.txt"

echo 'PASS: prepared M1.3b.4b.5r runtime'
