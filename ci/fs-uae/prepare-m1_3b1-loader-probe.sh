...
rm -f "$OUT/aros-root/m1-3b1-"*.marker "$OUT/aros-root/m68kdeb-loader-report.txt"

cat > "$OUT/aros-root/S/Startup-Sequence" <<'EOF'
C:Echo "M1.3b.1 Startup-Sequence reached" >SYS:m1-3b1-startup.marker
SYS:m68kdeb-loader >SYS:m68kdeb-loader-report.txt
C:Echo "M1.3b.1 loader returned" >SYS:m1-3b1-loader-returned.marker
C:Wait 300
EOF

{
  ...
  echo "loader_role=read-only Exec memory-map probe"
  echo "amiboot=absent"
  echo "linux=absent"
  sha256sum "$OUT/rom/aros-rom.bin" "$OUT/rom/aros-ext.bin" "$OUT/aros-root/m68kdeb-loader" "$OUT/aros-root/S/Startup-Sequence"
} > "$OUT/RUNTIME_PROVENANCE.txt"

echo 'PASS: prepared M1.3b.1 AROS-native loader probe'
