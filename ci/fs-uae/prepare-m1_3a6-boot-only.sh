...
rm -f "$OUT/aros-root/m1-3a6-startup.marker"

cat > "$OUT/aros-root/S/Startup-Sequence" <<'EOF'
C:Echo "M1.3a.6 AROS A1200 68EC030 Startup-Sequence reached" >SYS:m1-3a6-startup.marker
C:Echo "m68kDeb M1.3a.6 no-MMU diagnostic control"
C:Wait 300
EOF

{
  ...
  echo "qualification_scope=diagnostic control only; 68EC030 is not a supported m68kDeb target"
  echo "control=M1.3a.5 generic 68030+MMU failed during AROS MMU init before Startup-Sequence"
  sha256sum "$OUT/aros-root/S/Startup-Sequence" "$OUT/ORIGINAL-Startup-Sequence"
} > "$OUT/RUNTIME_PROVENANCE.txt"

echo "PASS: prepared pinned AROS A1200 68EC030 no-MMU diagnostic root"
