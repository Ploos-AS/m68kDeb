...
rm -f "$OUT/aros-root/m1-3a13-"*.marker

cat > "$OUT/aros-root/S/Startup-Sequence" <<'EOF'
C:Echo "M1.3a.13 Startup-Sequence reached" >SYS:m1-3a13-startup.marker
C:Stack 100000
C:Echo "M1.3a.13 pre-handoff boundary reached" >SYS:m1-3a13-pre-handoff.marker
SYS:amiboot -d -k SYS:vmlinux-m68k-amiga root=/dev/ram video=pal console=ttyS0,9600n8
C:Echo "M1.3a.13 amiboot returned to AROS" >SYS:m1-3a13-amiboot-returned.marker
C:Wait 300
EOF

{
  ...
  echo "probe=observe pre-handoff, amiboot return, Linux serial, emulator exit, and host-side payload metadata"
  printf 'amiboot_bytes='; wc -c < "$OUT/aros-root/amiboot"
  printf 'vmlinux_bytes='; wc -c < "$OUT/aros-root/vmlinux-m68k-amiga"
...
echo "PASS: prepared M1.3a.13 amiboot/Linux handoff diagnostic"
