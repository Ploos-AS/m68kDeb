...
rm -f "$OUT/aros-root/m1-3a7-startup.marker"

cat > "$OUT/aros-root/S/Startup-Sequence" <<'EOF'
C:Echo "M1.3a.7 AROS A1200 68040+MMU Startup-Sequence reached" >SYS:m1-3a7-startup.marker
C:Echo "m68kDeb M1.3a.7 68040+MMU boot-only probe"
C:Wait 300
EOF

{
  ...
  echo "control=M1.3a.5 68030+MMU failed; M1.3a.6 68EC030/no-MMU passed"
  sha256sum "$OUT/aros-root/S/Startup-Sequence" "$OUT/ORIGINAL-Startup-Sequence"
} > "$OUT/RUNTIME_PROVENANCE.txt"

echo "PASS: prepared pinned AROS A1200 68040+MMU boot-only root"
