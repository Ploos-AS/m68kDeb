...
PROBE=${PROBE:-out/m1-3b4b5q/m68kdeb-68030-trampoline-window-runtime}
OUT=${OUT:-out/m1-3b4b5q}
...
cp "$PROBE" "$OUT/aros-root/m68kdeb-68030-trampoline-window-runtime"
rm -f "$OUT/aros-root/m1-3b4b5q-"*.marker "$OUT/aros-root/m68kdeb-5q-report.txt"

cat > "$OUT/aros-root/S/Startup-Sequence" <<'EOF'
C:Echo "M1.3b.4b.5q Startup-Sequence reached" >SYS:m1-3b4b5q-startup.marker
SYS:m68kdeb-68030-trampoline-window-runtime >SYS:m68kdeb-5q-report.txt
C:Echo "M1.3b.4b.5q probe returned" >SYS:m1-3b4b5q-returned.marker
C:Wait 300
EOF

{
  ...
  echo "cpu=68030"
  echo "mmu=68030"
  echo "role=M1.3b.4b.5q reversible allocated trampoline identity window"
  echo "trampoline_copy=not_attempted"
  echo "trampoline_execute=not_attempted"
  echo "linux_handoff=not_attempted"
  sha256sum "$OUT/rom/aros-rom.bin" "$OUT/rom/aros-ext.bin" "$OUT/aros-root/m68kdeb-68030-trampoline-window-runtime"
} > "$OUT/RUNTIME_PROVENANCE.txt"

echo 'PASS: prepared M1.3b.4b.5q runtime'
