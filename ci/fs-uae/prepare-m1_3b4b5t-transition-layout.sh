...
PROBE=${PROBE:-out/m1-3b4b5t/m68kdeb-68030-transition-layout-runtime}
OUT=${OUT:-out/m1-3b4b5t}
...
cp "$PROBE" "$OUT/aros-root/m68kdeb-68030-transition-layout-runtime"
rm -f "$OUT/aros-root/m1-3b4b5t-"*.marker "$OUT/aros-root/m68kdeb-5t-report.txt"

cat > "$OUT/aros-root/S/Startup-Sequence" <<'EOF'
C:Echo "M1.3b.4b.5t Startup-Sequence reached" >SYS:m1-3b4b5t-startup.marker
SYS:m68kdeb-68030-transition-layout-runtime >SYS:m68kdeb-5t-report.txt
C:Echo "M1.3b.4b.5t probe returned" >SYS:m1-3b4b5t-returned.marker
C:Wait 300
EOF

{
  ...
  echo "cpu=68030"
  echo "mmu=68030"
  echo "role=M1.3b.4b.5t reversible transition layout binding"
  ...
  sha256sum "$OUT/rom/aros-rom.bin" "$OUT/rom/aros-ext.bin" "$OUT/aros-root/m68kdeb-68030-transition-layout-runtime"
} > "$OUT/RUNTIME_PROVENANCE.txt"

echo 'PASS: prepared M1.3b.4b.5t runtime'
