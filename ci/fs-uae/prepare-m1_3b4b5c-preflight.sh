...
rm -f "$OUT/aros-root/m1-3b4b5c-"*.marker "$OUT/aros-root/m68kdeb-preflight-report.txt"

cat > "$OUT/aros-root/S/Startup-Sequence" <<'EOF'
C:Echo "M1.3b.4b.5c Startup-Sequence reached" >SYS:m1-3b4b5c-startup.marker
SYS:m68kdeb-loader SYS:vmlinux-m68k-amiga SYS:initramfs-m68kdeb.gz 5 >SYS:m68kdeb-preflight-report.txt
C:Echo "M1.3b.4b.5c reversible preflight returned" >SYS:m1-3b4b5c-returned.marker
C:Wait 300
EOF

{
  ...
  echo "role=M1.3b.4b.5c reversible takeover preflight"
...
echo 'PASS: prepared M1.3b.4b.5c reversible runtime'
