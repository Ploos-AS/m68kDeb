...
for f in amiboot woody-linux.bin; do
...
rm -f "$OUT/aros-root/m1-3a14-"*.marker

cat > "$OUT/aros-root/S/Startup-Sequence" <<'EOF'
C:Echo "M1.3a.14 Startup-Sequence reached" >SYS:m1-3a14-startup.marker
C:Stack 100000
C:Echo "M1.3a.14 pre-handoff boundary reached" >SYS:m1-3a14-pre-handoff.marker
SYS:amiboot -d -k SYS:woody-linux.bin root=/dev/ram video=pal console=ttyS0,9600n8
C:Echo "M1.3a.14 amiboot returned to AROS" >SYS:m1-3a14-amiboot-returned.marker
C:Wait 300
EOF

{
  ...
  echo "payload=amiboot + Debian 3.0 woody m68k Amiga linux.bin on DH0"
  echo "initramfs=absent"
  echo "probe=historical known-good Debian Amiga kernel control for amiboot/Linux handoff"
  printf 'amiboot_bytes='; wc -c < "$OUT/aros-root/amiboot"
  printf 'woody_kernel_bytes='; wc -c < "$OUT/aros-root/woody-linux.bin"
  sha256sum "$OUT/rom/aros-rom.bin" "$OUT/rom/aros-ext.bin" \
    "$OUT/aros-root/amiboot" "$OUT/aros-root/woody-linux.bin" \
    "$OUT/aros-root/S/Startup-Sequence" "$OUT/ORIGINAL-Startup-Sequence"
} > "$OUT/RUNTIME_PROVENANCE.txt"

echo "PASS: prepared M1.3a.14 Debian woody kernel handoff probe"
