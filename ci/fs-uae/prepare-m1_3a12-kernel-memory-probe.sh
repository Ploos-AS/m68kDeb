...
rm -f "$OUT/aros-root/m1-3a12-startup.marker" "$OUT/aros-root/m1-3a12-invoking-kernel.marker"

cat > "$OUT/aros-root/S/Startup-Sequence" <<'EOF'
C:Echo "M1.3a.12 Startup-Sequence reached" >SYS:m1-3a12-startup.marker
C:Stack 100000
C:Echo "M1.3a.12 invoking amiboot kernel-only probe with 64M FastRAM" >SYS:m1-3a12-invoking-kernel.marker
SYS:amiboot -d -k SYS:vmlinux-m68k-amiga root=/dev/ram video=pal console=ttyS0,9600n8
C:Echo "M1.3a.12 amiboot returned unexpectedly"
C:Wait 300
EOF

{
  ...
  echo "probe=repeat M1.3a.11 with explicit 64 MiB FastRAM and observe Linux kernel banner"
...
echo "PASS: prepared M1.3a.12 single-DH0 AROS root with explicit 64 MiB FastRAM"
