...
cp "$LOADER" "$OUT/aros-root/m68kdeb-loader-68030-precommit-qual"
cp "$KERNEL" "$OUT/aros-root/vmlinux-m68k-amiga"
cp "$INITRAMFS" "$OUT/aros-root/initramfs-m68kdeb.gz"
rm -f "$OUT/aros-root/m1-3b4b5y-"*.marker "$OUT/aros-root/m68kdeb-5y-report.txt"

cat > "$OUT/aros-root/S/Startup-Sequence" <<'EOF'
C:Echo "M1.3b.4b.5y Startup-Sequence reached" >SYS:m1-3b4b5y-startup.marker
SYS:m68kdeb-loader-68030-precommit-qual SYS:vmlinux-m68k-amiga SYS:initramfs-m68kdeb.gz 5 >SYS:m68kdeb-5y-report.txt
C:Echo "M1.3b.4b.5y loader returned" >SYS:m1-3b4b5y-returned.marker
C:Wait 300
EOF
...
echo 'PASS: prepared M1.3b.4b.5y reversible precommit runtime'
