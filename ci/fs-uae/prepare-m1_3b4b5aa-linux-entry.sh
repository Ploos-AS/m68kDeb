...
cp "$LOADER" "$OUT/aros-root/m68kdeb-loader-68030-linux-entry-qual"
cp "$KERNEL" "$OUT/aros-root/vmlinux-m68k-amiga"
cp "$INITRAMFS" "$OUT/aros-root/initramfs-m68kdeb.gz"
rm -f "$OUT/aros-root/m1-3b4b5aa-"*.marker "$OUT/aros-root/m68kdeb-5aa-report.txt"

cat > "$OUT/aros-root/S/Startup-Sequence" <<'EOF'
C:Echo "M1.3b.4b.5aa Startup-Sequence reached" >SYS:m1-3b4b5aa-startup.marker
SYS:m68kdeb-loader-68030-linux-entry-qual SYS:vmlinux-m68k-amiga SYS:initramfs-m68kdeb.gz 5 >SYS:m68kdeb-5aa-report.txt
C:Echo "M1.3b.4b.5aa loader returned unexpectedly" >SYS:m1-3b4b5aa-returned.marker
C:Wait 300
EOF

{
  ...
  echo "role=M1.3b.4b.5aa CI-only Linux entry transfer"
  echo "transition_entry_transfer=attempted_after_runtime_gate"
  echo "cache_mutation=contained_in_emulator"
  echo "mmu_mutation=contained_in_emulator"
  echo "pflusha=contained_in_emulator"
  echo "linux_entry_transfer=attempted"
  sha256sum "$OUT/rom/aros-rom.bin" "$OUT/rom/aros-ext.bin" \
    "$OUT/aros-root/m68kdeb-loader-68030-linux-entry-qual" \
...
echo 'PASS: prepared M1.3b.4b.5aa Linux-entry runtime'
