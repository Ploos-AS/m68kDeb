...
PROBE=${PROBE:-out/m1-3b4b5s/m68kdeb-68030-trampoline-entry-runtime}
OUT=${OUT:-out/m1-3b4b5s}
...
cp "$PROBE" "$OUT/aros-root/m68kdeb-68030-trampoline-entry-runtime"
rm -f "$OUT/aros-root/m1-3b4b5s-"*.marker "$OUT/aros-root/m68kdeb-5s-report.txt"

cat > "$OUT/aros-root/S/Startup-Sequence" <<'EOF'
C:Echo "M1.3b.4b.5s Startup-Sequence reached" >SYS:m1-3b4b5s-startup.marker
SYS:m68kdeb-68030-trampoline-entry-runtime >SYS:m68kdeb-5s-report.txt
C:Echo "M1.3b.4b.5s probe returned" >SYS:m1-3b4b5s-returned.marker
C:Wait 300
EOF

{
  ...
  echo "cpu=68030"
  echo "mmu=68030"
  echo "role=M1.3b.4b.5s reversible copied trampoline probe entry"
  echo "transition_entry=not_attempted"
  echo "mmu_mutation=not_attempted"
  echo "cache_mutation=not_attempted"
  echo "linux_handoff=not_attempted"
  sha256sum "$OUT/rom/aros-rom.bin" "$OUT/rom/aros-ext.bin" "$OUT/aros-root/m68kdeb-68030-trampoline-entry-runtime"
} > "$OUT/RUNTIME_PROVENANCE.txt"

echo 'PASS: prepared M1.3b.4b.5s runtime'
